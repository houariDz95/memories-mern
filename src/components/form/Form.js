...
const Form = ({ currentId, setCurrentId }) => {
  const [postData, setPostData] = useState({ title: '', message: '', tags: '', selectedFile: '',  createdAt: new Date()});
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if(currentId){
      dispatch(updatePost(currentId, {...postData, name: user?.displayName}));  
    }else {
      dispatch(createPost({ ...postData, name: user?.displayName}));
    ...
    return (
      <Paper className={classes.paper}>
        <Typography variant="h6" >
          Pleas Sign In to create your own memories
        </Typography>
      </Paper>
    )
  }
